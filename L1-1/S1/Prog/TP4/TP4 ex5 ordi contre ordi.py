"""
Variante de l'exercice 5 du TP4, mais sans interaction humaine, avec
un nombre défini de tour
"""

import random

nbDeTour = 1000 #le nb de tour
...
for tour in range(nbDeTour): #répète en f() du nb de tour, en gros ça représente un tour en entier
    ...
    nbDeDes = random.randint(2, 6) #Le nb de dés que le joueur et l'ordinateur vont lancer
    print("On va lancer,", nbDeDes, "dés")
    print("\n") #retour à la ligne

    resJoueur = random.randint((1*nbDeDes), (6*nbDeDes)) #lancement du joueur
    print("Jean-Michel ", resJoueur)
    ...
    if (resJoueur > 21) or (resOrdi > 21): #si l'un des deux sont au dessus de 21
        if resJoueur > 21: #si c'est le joueur qui est au dessus
            print("Jean-Michel a dépassé 21, il perd donc deux points")
            scoreJoueur -= 2
        else: #sinon l'ordi
        ...
        if resJoueur == 21: #si c'est le joueur
            print("Jean-Michel a 21 tout pile, il gagne donc 5 points")
            scoreJoueur += 5
        else: #sinon l'ordi
        ...
        if max(tupleRes) == resJoueur: #si le plus proche de 21 est le joueur
            print("Jean-Michel est le plus proche de 21, il gagne donc 2 points!")
            scoreJoueur += 2
